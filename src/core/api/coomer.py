...
class CoomerClient(BaseAPIClient):
    BASE_URL = "https://coomer.st/api"
    PLATFORM = "coomer"
    ...
    def get_all_creators(self) -> List[dict]:
        # Endpoint requires Accept: text/css but returns JSON
        data = self._request(
            "GET",
            "/v1/creators",
    ...
    def get_creator_posts(
        ...
        *,
        ...
        # Best-effort creator info: coomer creator_posts payload includes props + artist
    ...
    def get_posts(
        ...
        *,
        ...
        self._logger.debug(f"get_posts params before request: {params}")
        ...
        count = int(data.get("count") or 0)           # capped at 50000
    ...
    def get_popular_posts(
        ...
        *,
        ...
        return {
            ...
            "posts": posts,
            "offset": offset,
            "has_more": (offset + len(posts)) < total if total else (len(posts) == int((props or {}).get("limit") or 50)),
        }
    ...
    def normalize_creator(self, raw: dict) -> dict:
        # creators list: id/name/service/indexed/updated/favorited (no post count)
        # creator profile: contains more fields and post counts (commonly "posts")
        return {
            "id": raw.get("id"),
